...
class com.fusioncharts.core.charts.Scatter2DChart extends XYPlotChart
{
	...
	*/
	private function returnDataAsObject (xv : Number, yv : Number, displayValue:String, toolText : String, link : String, showValue : Number) : Object
	{
		//Create a container
					...
					} else if (arrLevel1Nodes [j].nodeName.toUpperCase () == "DATASET")
					{
						...
						for (k = 0; k < arrLevel2Nodes.length; k ++)
						{
							if (arrLevel2Nodes [k].nodeName.toUpperCase () == "SET")
							{
								...
								this.dataset [this.numDS].data [setCount] = this.returnDataAsObject (setXV, setYV,  setExDispVal, setToolText, setLink, setShowValue);
							}
						}
	...
	*/
	...
	public function exportChartDataCSV():String {
		...
		var i:Number, j:Number;
		strData = strQ + "Series" + strQ + strS + strQ + "x"  + strQ + strS + strQ + "y" + strQ + strLB;
		//Iterate through each data-item and add it to the output
		for (i = 1; i <= this.numDS; i ++)
		{
			for (j = 1; j <= this.dataset [i].num; j ++)
			{
				strData += strQ + ((this.dataset[i].seriesName != "")?(this.dataset[i].seriesName):(""))  + strQ + strS + strQ + ((this.dataset[i].data[j].isDefined == true)?(this.dataset[i].data[j].xv):(""))  + strQ + strS + strQ + ((this.dataset[i].data[j].isDefined==true)?((this.params.exportDataFormattedVal==true)?(this.dataset[i].data[j].formattedValue):(this.dataset[i].data[j].yv)):(""))  + strQ + ((i==this.numDS && j==this.dataset [i].num)?"":strLB);
			}
		}
		return strData;
	}
