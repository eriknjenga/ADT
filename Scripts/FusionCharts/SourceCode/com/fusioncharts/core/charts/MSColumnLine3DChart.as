...
class com.fusioncharts.core.charts.MSColumnLine3DChart extends SingleYAxis3DChart 
{
	...
	*/
	function MSColumnLine3DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		_arrObjects = new Array ("BACKGROUND", "CANVAS", "CAPTION", "SUBCAPTION", "YAXISNAME", "XAXISNAME", "DIVLINES", "YAXISVALUES", "DATALABELS", "DATAVALUES", "LEGEND", "TRENDLINES", "TRENDVALUES", "DATAPLOTCOLUMN", "DATAPLOTLINE", "TOOLTIP", "VLINES", "ANCHORS", "VLINELABELS");
		...
		for (i = 1; i <= this.numDS; i ++)
		{
			if (this.dataset [i].renderAs != "COLUMN")
			{
				for (j = 1; j <= this.num; j ++)
				{
					...
					//Set the y position
					this.dataset [i].data [j].y = this.getAxisPosition (this.dataset [i].data [j].value, this.config.yMax, this.config.yMin, this.elements.canvasBg.y, this.elements.canvasBg.toY, true, 0) + (this.params.use3DLineShift?this.config.shifts.yShift:0);
					//Store value textbox y position
